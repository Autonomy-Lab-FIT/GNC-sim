...
% implementation of LQ tracker
    ...
    % weights for Q and R matrices
    config_lqt = struct();
    config_lqt.weights.position = 2;
    config_lqt.weights.velocity = 2; 
    config_lqt.weights.att = 2;
    config_lqt.weights.ang_vel = 10;
    config_lqt.control_weights = [5, 20, 20, 20];
    ...
    params.radius = 5;
    params.altitude = config.takeoff_altitude;
    params.period = 25;
    params.dt = dt_dyn;
    params.duration = flight_duration;
    ...
    fprintf('Starting lqt Simulation...\n');
    ...
    x_initial = state_vec(telemetry);
    params.start_angle = atan2(x_initial(2), x_initial(1));

    trajectory = generate_circular_trajectory(params);

    % switch to control mode to our controller
    px4_switch_control_mode(client, CONTROL_MODE, config);
    pause(2);  % Allow mode transition
    
    fprintf('Starting lqt control...\n');
    ...
    u_prev = U_ff;
     ...
     while t < flight_duration
        ...
        % A and B depend on current state
        [A, B] = drone_linear_dynamics(x_curr, u_prev, px4_config); 
        [K, ~] = compute_k_matrix(A, B, config_lqt, px4_config); 
        
        % control law
        x_err = x_curr - x_ref;

        q_err = quatmultiply(quatinv(x_ref(7:10)'), x_curr(7:10)');
        if q_err(1) < 0
            q_err = -q_err;
        end
        x_err(7:10) = q_err' - [1; 0; 0; 0];

        u_lqt = U_ff - K*x_err; 
        
        % saturate the control inputs
        u_lqt_sat = saturate_control(u_lqt, px4_config);
       
        % Nonlinear dynamics integration
        dynamics_func = @(t, x) drone_nonlinear_dynamics(t, x, u_lqt_sat, px4_config);
        x_next = RK4(dynamics_func, x_curr, dt_dyn, t);
        
        % Normalize the predicted quaternion
        x_next(7:10) = x_next(7:10) / norm(x_next(7:10));
        
        thrust_cmd = u_lqt_sat(1);
        tau_cmd = u_lqt_sat(2:4);
        ...
        % px4_send_rate_setpoint(client, u_lqt(1), x_next(11), x_next(12), x_next(13), config);
        % px4_send_trajectory(client, x_next(1), x_next(2), x_next(3), 0 , config)
       ...
       if mod(round(t/dt_dyn), 25) == 0
            ...
            fprintf('| T=%.1fN τ=[%.3f,%.3f,%.3f]Nm', ...
                    u_lqt_sat(1), tau_cmd(1), tau_cmd(2), tau_cmd(3));
            if u_lqt_sat(1) ~= u_lqt(1), fprintf(' [T_SAT]'); end
            if any(abs(u_lqt(2:4)) > 1.5), fprintf(' [τ_SAT]'); end
            fprintf('\n');
        end

        % Logging and Timing
        log_data = update_log(log_data, t, x_curr, x_err, u_lqt_sat);
        t = t + dt_dyn;
        u_prev(1) = u_lqt_sat(1);
        u_prev(2) = u_lqt_sat(2);
        u_prev(3) = u_lqt_sat(3);
        u_prev(4) = u_lqt_sat(4);
    ...
    save_log_data(log_data, 'lqt_control_log.mat');
