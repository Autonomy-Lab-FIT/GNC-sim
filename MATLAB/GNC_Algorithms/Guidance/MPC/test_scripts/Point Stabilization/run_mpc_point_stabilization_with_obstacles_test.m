function run_mpc_point_stabilization_with_obstacles_test(config, client)
%RUN_MPC_POINT_STABILIZATION_TEST Complete UKF + MPC Point Stabilization test

    % USER-DEFINED TARGET POSITION
    target_position = [0, 3.5, -1.5];  % [x, y, z] - modify this if needed, NED frame
    ...
    while t < flight_duration && step <= num_steps
        ...
        % LiDAR data
        obstacle_data = px4_get_lidar_scan(client, config);

        % MPC Point Stabilization Controller
        [pos_cmd, vel_cmd, mpc_state] = mpc_point_stabilization_with_obstacles(...
            target_position, x_ukf, mpc_config, mpc_state, obstacle_data);
